...
Make sure you have created a directory for today's practical (e.g., `2020-09-xx-read_cleaning`) and the `input`, `tmp`, and `results` subdirectories. Link the raw sequence files (`/shared/data/reads.pe*.fastq.gz`) into `input` subdirectory:
